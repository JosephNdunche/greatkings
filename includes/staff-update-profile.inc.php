...
if(isset($_POST['staff-update-profile'])){
    ...
    if(count($error) === 0){
        ...
        $sql = "UPDATE staff SET unique_id=?, surname=?, other_names=?, email=?, telephone=?, class=?, state_of_origin=?, date_of_birth=?, home_address=?, updated=NOW() WHERE unique_id=? LIMIT 1";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('ssssssssss', $staff_id, $surname, $other_names, $email, $telephone, $class, $state_of_origin, $date_of_birth, $address, $staff_id);
        if($stmt->execute()){
