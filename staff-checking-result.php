...
<table class="table table-responsive">
    ...
    <td>'. $row['grade'] .'</td>
    <td><a href="staff-update-result2.php?unique_id='.$row['student_id'].'&student_course='.$row['course'].'&student_session='.$row['session'].'&student_term='.$row['term'].'&student_first_term_score='.$row['first_term_score'].'&student_second_term_score='.$row['second_term_score'].'"><i class="bi bi-pencil-square"></i></a></td>
    </tr>
    ';
    }
    $stmt->close();
...
<button  class="btn btn-success text-white">Print Result</button>
</div>
<?php
/*if($ave_score >= 70){
    $grade = "A";
